...
describe('Aggregator - 边界条件补充', () => {
  it('仅 1 张卡片的场景', () => {
    const queue = buildReviewQueue([card('c1', 'ai', null, 0)]);
    expect(queue).toEqual(['c1']);
  });

  it('maxCount = 0 返回空数组', () => {
    const queue = buildReviewQueue([card('c1', 'ai', null, 0)], 0);
  ...
  it('大量卡片（100 张）性能测试', () => {
    const cards: DueCard[] = Array.from({ length: 100 }, (_, i) =>
      card(`c${i}`, i % 3 === 0 ? 'ai' : i % 3 === 1 ? 'blockchain' : 'general', null, 0)
    );
  ...
  it('round-robin 确保公平分配', () => {
    ...
    const cards: DueCard[] = [
      ...Array.from({ length: 10 }, (_, i) => card(`a${i}`, 'ai', null, 0)),
      ...Array.from({ length: 10 }, (_, i) => card(`b${i}`, 'blockchain', null, 0)),
    ];
